...
settingsRoutes.post('/plex', async (req, res, next) => {
  ...
  try {
    ...
    if (!admin.plexToken) {
      throw new Error(
        'The instance administrator must have their account connected to Plex to be able to set up a Plex server.'
      );
    }
...
settingsRoutes.get('/plex/devices/servers', async (req, res, next) => {
  ...
  try {
    ...
    if (!admin.plexToken) {
      throw new Error('Plex must be configured on the instance to retrieve servers.');
    }
...
settingsRoutes.get('/plex/library', async (req, res, next) => {
  ...
  if (req.query.sync) {
    ...
    if (!admin.plexToken) {
      return next({
        status: '500',
        message: 'Plex must be configured on the instance to retrieve libraries.',
      });
    }
